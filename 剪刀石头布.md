...
1. 第三个进入的人的钱就消失了。
2. 可以窥测对手的input。（input是明文传输，并且记录在区块链中）
...
def open(choice, nonce):# 公开结果
	if not num_players == 2: return(-1) # Determine which player is opening 
	...
	if sha3([msg.sender, choice, nonce], items=3) == player[player_num].commit and not
...
恶意的参与者一直选择不公开，这2000钱就永久的被劫持了。



解决办法，给设定ddl，必须公开。具体应该是这样的

A，B提交加密 input 到合约，A，B 分别在ddl前解密， 计算结果。
